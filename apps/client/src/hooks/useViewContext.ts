...
import { viewContextAtom, type ViewContext, saveViewContext, loadViewContext } from '@/stores/view-context';
import { apiClient } from '@/lib/api-client';

export function useViewContext() {
  const [ctx, setCtx] = useAtom(viewContextAtom);

  const restore = useCallback((token: string) => {
  ...
  const update = useCallback((updater: (prev: ViewContext | null) => ViewContext | null) => {
    setCtx(prev => {
  ...
  const set = useCallback((next: ViewContext) => {
    setCtx(next);
  ...
  const neighbors = useMemo(() => {
    if (!ctx) return { prevId: undefined as number | undefined, nextId: undefined as number | undefined };
    return {
  ...
  const prefetchAround = useCallback(async (datasetId: string | number, windowSize = 1) => {
    if (!ctx) return;
    const promises: Promise<any>[] = [];
    for (let i = Math.max(0, ctx.currentIndex - windowSize); i <= Math.min(ctx.ids.length - 1, ctx.currentIndex + windowSize); i++) {
      const id = ctx.ids[i];
      promises.push(apiClient.getStack(id, datasetId));
    }
    try { await Promise.all(promises); } catch { /* ignore */ }
  }, [ctx]);
