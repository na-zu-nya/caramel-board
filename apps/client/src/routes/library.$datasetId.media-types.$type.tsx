...
export const Route = createFileRoute('/library/$datasetId/media-types/$type')({
  component: MediaTypesTypeList,
});

function MediaTypesTypeList() {
  const t = useT();
  const { datasetId, type } = Route.useParams();
  ...
  useEffect(() => {
    ...
    const newFilter: StackFilter = {
      datasetId,
      mediaTypes: search.mediaTypes ?? [type as MediaType],
  ...
  }, [total, datasetId, navigate, currentFilter, currentSort, search, allItems, loadPage]);
  ...
  const handleFilterChange = useCallback(
    ...
    [setCurrentFilter, setNavigationState, navigate, datasetId, type]
  );
  ...
  const handleItemClick = useCallback(
    ...
    [navigate, datasetId, search, setNavigationState, total, allItems, currentFilter, currentSort]
  );
