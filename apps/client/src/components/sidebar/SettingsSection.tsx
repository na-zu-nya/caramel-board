    ...
    <SideMenuGroup label="Settings" collapsed={isCollapsed} onToggle={onToggle}>
      <nav className="space-y-0.5">
        ...
        <SideMenuListItem asChild icon={Tag} label="Tags">
          <Link to="/library/$datasetId/tags" params={() => ({ datasetId })} activeProps={{ className: 'bg-gray-100 font-medium' }} />
        </SideMenuListItem>
        <SideMenuListItem asChild icon={Wand2} label="Auto-Tag">
          <Link to="/library/$datasetId/autotag-config" params={() => ({ datasetId })} activeProps={{ className: 'bg-gray-100 font-medium' }} />
        </SideMenuListItem>
        <SideMenuListItem asChild icon={PinIcon} label="Pins">
          <Link to="/library/$datasetId/pins" params={() => ({ datasetId })} activeProps={{ className: 'bg-gray-100 font-medium' }} />
        </SideMenuListItem>
      </nav>
    </SideMenuGroup>
  );
