...
export default function TapZoneOverlay({
  ...
  useEffect(() => {
    ...
    const isInteractiveElement = (el: Element | null): boolean => {
      ...
      while (cur && depth < 6) {
        ...
        if (
          ...
          tag === 'LABEL' ||
          role === 'button' ||
          cur.hasAttribute('data-interactive') ||
          cur.hasAttribute('data-no-tap') ||
          cur.classList.contains('ui-control')
        ) {
          return true;
        }
    ...
    const handlePointerDown = (e: PointerEvent) => {
      // Check if click is on a video element - if so, don't interfere
      const target = e.target as Element;
      if (target && (target.tagName === 'VIDEO' || isInteractiveElement(target))) {
        return; // Let video handle its own click events
      }

      // Check if the pointer is within the content area
      const isWithinContentArea =
        e.clientX >= contentArea.left &&
        e.clientX <= window.innerWidth - contentArea.right &&
        e.clientY >= contentArea.top &&
        e.clientY <= window.innerHeight - contentArea.bottom;

      // Only track if no other pointer is active and within content area
      if (activePointerRef.current === null && isWithinContentArea) {
        activePointerRef.current = e.pointerId;
        startPosRef.current = {
          x: e.clientX,
          y: e.clientY,
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (activePointerRef.current === e.pointerId && startPosRef.current) {
        const deltaX = e.clientX - startPosRef.current.x;
        const deltaY = e.clientY - startPosRef.current.y;
        const absDeltaX = Math.abs(deltaX);
        const absDeltaY = Math.abs(deltaY);

        // Determine drag direction after threshold is met
        if (
          !isDraggingRef.current &&
          (absDeltaX > DIRECTION_LOCK_THRESHOLD || absDeltaY > DIRECTION_LOCK_THRESHOLD)
        ) {
          ...
          // Lock to the direction with larger movement
        ...
        if (isDraggingRef.current) {
          if (dragDirectionRef.current === 'horizontal' && onDrag) {
            // Horizontal drag - existing behavior
            const dragDelta = e.clientX - lastDragXRef.current;
            lastDragXRef.current = e.clientX;
            onDrag(dragDelta);
          } else if (dragDirectionRef.current === 'vertical' && onVerticalDrag) {
            // Vertical drag - new behavior
            const dragDelta = e.clientY - lastDragYRef.current;
            lastDragYRef.current = e.clientY;

            // Calculate progress for scale/opacity (0 = top, 1 = bottom of screen)
            const screenHeight = window.innerHeight;
            const currentY = e.clientY;
            const startY = startPosRef.current.y;

            // Only process downward movement for scaling/fading
            if (deltaY > 0) {
              const progress = Math.min(
                1,
                Math.max(0, (currentY - startY) / (screenHeight - startY))
              );
              onVerticalDrag(dragDelta, progress);
            }
          }
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      if (activePointerRef.current === e.pointerId && startPosRef.current) {
        ...
        // If was dragging, handle drag end based on direction
        if (isDraggingRef.current) {
          if (dragDirectionRef.current === 'horizontal' && onDragEnd) {
          ...
          // Ignore taps on interactive elements (safety check on pointerup too)
          const target = e.target as Element;
        ...
        // Release pointer capture and reset tracking
        // No pointer capture used
        activePointerRef.current = null;
        startPosRef.current = null;
        isDraggingRef.current = false;
        dragDirectionRef.current = null;
