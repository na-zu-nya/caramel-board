...
- 完全にローカル動作・外部送信なしで、プライベートな管理
- 複数ライブラリ、タグ・作者設定、色抽出、(オプション)自動タグ付け、お気に入りで強力に検索・切り出し

#### 構築の難しさ

- **Web開発者:** ★★⭐︎⭐︎⭐︎ (比較的かんたんです)
- **PCに慣れてる人:** ★★★⭐︎⭐︎ (ややむずかしいです)
...
- **第三者向けのホステッド／マネージドサービスとしての提供はできません**（SaaS不可です。必要な場合は個別許諾をご相談ください）。
- 本ソフトは**現状有姿（AS IS）**で提供され、明示・黙示を問わずいかなる保証も行いません。

特にユーザ制限など設定してないため、インターネット上に公開するのはとっても危険です。管理できる範囲内で運用してください。
...
- 貢献: Issue は歓迎します。PR は承認済みメンテナのみ受け付けます
- 開発者向けの詳しい手順は docs/DEVELOPMENT.md を参照してください
